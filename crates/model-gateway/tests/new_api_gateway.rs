...
fn new_api_config_honors_environment_overrides() {
    let config = ModelGatewayConfig::from_env_with(|key| match key {
        ...
        "MY_SIFU_LLM_GATEWAY_API_KEY" => Some("local-test-token".to_string()),
...
#[test]
fn chat_request_builder_prepares_new_api_http_request_without_calling_provider() {
    let config = ModelGatewayConfig::from_env_with(|key| match key {
        "MY_SIFU_LLM_GATEWAY_BASE_URL" => Some("http://127.0.0.1:3000/".to_string()),
        "MY_SIFU_LLM_GATEWAY_API_KEY" => Some("local-test-token".to_string()),
        "MY_SIFU_DEFAULT_MODEL" => Some("gpt-4o-mini".to_string()),
        _ => None,
    });
    let request = ChatCompletionRequest {
        messages: vec![ChatCompletionMessage::user("检查 New API 请求边界。")],
        temperature: None,
        stream: false,
        model: None,
    };

    let http_request = NewApiChatRequestBuilder::new(config).build_http_request(request);
    ...
    assert_eq!(http_request.body["model"], "gpt-4o-mini");
    assert_eq!(http_request.body["messages"][0]["role"], "user");
}
